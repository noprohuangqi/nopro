...
SEED = 777
SHAPE = (192, 192, 3)
DIR = 'E:\\11_16_data_clean\\baseline_small_data_test'
VAL_RATIO = 0.2 # 10 % as validation
...
def getTrainDataset():
    ...
    for name, lbl in zip(data['path_new'], data['TARGET'].str.split(' ')):
        y = np.zeros(18)
        for key in lbl:
            y[int(key)] = 1
...
bestModel = load_model('./base.model', custom_objects={'f1':f1,'tp':tp}) #, 'f1_loss': f1_loss})

fullValGen = vg
lastFullValPred = np.empty((0, 18))
lastFullValLabels = np.empty((0, 18))
...
submit = pd.read_csv(DIR + '/test.csv')
P = np.zeros((pathsTest.shape[0], 18))
...
submit.to_csv('predict.csv', encoding = 'utf_8_sig',index=False)
