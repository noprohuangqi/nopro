...
for j in range(2):
    ...
    for train_index,test_index in kfold:
        ...
        dtest_predictions = svc.predict(x_test)
        dtest_predprob = svc.predict_proba(x_test)[:,:]
        print ("Classifier Accuracy : %.4g" % metrics.accuracy_score(y_test, dtest_predictions))
        ...
        result['predict'] = dtest_predictions
        result['result'] = y_test
        result.reset_index()
        pred_prob = pd.DataFrame(dtest_predprob, columns=['NORMAL','BAD'])
        ...
        result.to_csv('C:\\Users\\32002\\Desktop\\ANA_inspection\\svm\\data\\result\\svm\\muti_svm_result_%d_%d.csv'%(j,i))
        
        total_tests.extend(list(y_test))
        total_predictions.extend(list(dtest_predictions))
